package io.sandbox.zones.gui_element;
...
public class Selection extends WButton {
	private static final Identifier BUTTON_TEXTURES = new Identifier("minecraft",
			"textures/gui/container/enchanting_table.png");
	public Text subText;
	public List<Icon> icons = new ArrayList<>();
	public List<Icon> hoverIcons = new ArrayList<>();
	...
	public void paint(MatrixStack matrices, int x, int y, int mouseX, int mouseY) {
		...
		ScreenDrawing.texturedRect(matrices, x, y, getWidth() / 2, height, texture, buttonLeft, buttonTop,
				buttonLeft + buttonWidth, buttonTop + buttonHeight, 0xFFFFFFFF);
		ScreenDrawing.texturedRect(matrices, x + (getWidth() / 2), y, getWidth() / 2, height, texture, buttonEndLeft,
				buttonTop, spriteWidth * px, buttonTop + buttonHeight, 0xFFFFFFFF);

		for (int i = 0; i < this.icons.size(); i++) {
			Icon icon = this.icons.get(i);
			if (state != 2 && this.hoverIcons.contains(icon)) {
				continue;
			}

			int pxShift = i * 16;
			icon.paint(matrices, x + 2 + pxShift, y + 1, 16);
		}
	...
	public void addHoverIcon(Icon icon) {
		this.icons.add(icon);
		this.hoverIcons.add(icon);
	}
